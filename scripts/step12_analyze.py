...
def main(argv=None) -> None:
    ...
    for idx, (question, profile, rp_arg) in enumerate(zip(queries, profiles, packs)):
        ...
        if is_uw:
            ...
            _raw_count = len(uw_result["conditions"])
            uw_result["conditions"] = _postprocess_conditions(
                uw_result["conditions"], debug=_DEBUG
            )
            _removed_count = _raw_count - len(uw_result["conditions"])
            if _raw_count > 0 and _removed_count / _raw_count > 0.30:
